...
<div class="response" >
    ...
    <div class="form-group mt-3" >
        {{ Form::textarea('description', null, ['class' => 'form-control richText-editor','rows' => 5,'placeholder' => __('Description'),'id'=>'richtext']) }}
...
<script>
    ...
    $('modal-body').ready(function(){
        ...
        else if( temp == 'comment' && $('.grammer_textarea_comment').length>0){

            var summernoteValue = $('.grammer_textarea_comment').val();
        }
        else if( temp == 'note' && $('.grammer_textarea_note').length>0){
    ...
    function copyGrammerText() {
        ...
        if( temp == 'comment' && $('.grammer_textarea_comment').length>0){
